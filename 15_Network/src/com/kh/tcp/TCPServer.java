 ...
 *    1. 서버, 클라이언트간에 1:1 소켓 통신
 *    2. 데이터를 교환하기에 앞서 서버, 클라이언트 연결이 되어야 한다.
 *      (서버가 먼저 실행되어 클라이언트
 *    3. 신뢰성 있는 데이터 전달 가능
 *    
 *  * Socket
 *    1. 프로세스간 통신 담당
 *    2. Input / OutputStream 을 가지고 있다(스트림을 통해 입출력 이루어진다).
 *    
 *  * ServerSocket
 *    1. 포트와 연결(Binding)되어 외부의 연결 요청을 기다린다
 *      -> 요청이 들어오면 수락해준다.
 *    2. 수락 -> 통신할 수 있는 socket 형성
 * 
 */
