...
// Controller : View를 통해 사용자가 요청한 기능을 처리하는 담당
//				해당 메소드로 전달된 데이터[가공처리 후] dao로 전달하며 호출
//				dao로부터 반환받은 결과 성공/실패 판단 후 응답화면 결정(view메소드 호출)
...
public class MemberController {
	 ...
	 * @param userId ~ hobby : 사용자가 입력했던 정보가 담겨있는 매개변수
	 */
	
	public void insertMember(String userId, String userPwd, String userName
							, String gender, String age, String email, String phone
							, String address, String hobby) {
		// view로부터 전달받은 값을 바로 dao쪽으로 전달 x
		// 어딘가(Member객체(vo))에 담아서 전달
		
		// 방법1) 기본생성자로 생성 후 각 필드마다 setter을 이용해서 저장
		// 방법2) 매개변수 있는 생성자로 전부 전달해 생성
	...
	/**
	 * 사용자의 아이디로 회원정보 검색요청을 처리해주는 메소두
	 * @param userId : 사용자가 직접입력한 검색하고자 하는 회원 아이디
	 * 
	 */
